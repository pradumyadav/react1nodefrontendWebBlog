...
export default function Fitness(){
    ...
    useEffect(()=>{
        axios.get("https://nodeblog20.onrender.com/api/fitness")
    ...
    return(
        ...
        <div className="fitnessData">
            ...
            {data.filter((item)=>item.ID>=51 && item.ID<59)
            .map((item,index)=>{
                return(
            ...
            {data.filter((item)=>item.ID ===64).map((item,index)=>{
                return(
                    <div key={index}>
            ...
            {data.filter((item)=>item.ID>=60 && item.ID<=64)
            .map((item,index)=>{
                return(
